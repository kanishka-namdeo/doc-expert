...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [totalUsers] = await db.select({ count: count() }).from(user).where(userConditions.length ? and(...userConditions) as any : undefined);
    const [totalConversations] = await db.select({ count: count() }).from(conversation).where(convConditions.length ? and(...convConditions) as any : undefined);
    const [totalAuditLogs] = await db.select({ count: count() }).from(auditLog).where(auditConditions.length ? and(...auditConditions) as any : undefined);
